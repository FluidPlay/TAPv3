...
return {
	en = {
		tasks = {
			...
			tips = {
				buildMex = 'Metal is used for construction and requires ore spots.',
		...
		steps = { -- I am Nubtron, the multilingual friendly robot.
			intro = [[Hello! I will teach you how to play TAP (Total Atomic Power). Just follow my instructions and learn at your own pace.
<(Click next to continue)>]],
			intro2 = [[Just follow my instructions and learn at your own pace. <(Click next to continue)>]],
			intro3 = [[Move the camera with the <arrow keys>, holding <middle mouse button> and moving mouse, or moving your cursor to the <screen edge>.
<(Click next to continue)>]],
			intro4 = [[Zoom the camera with your mouse's <scroll wheel> or <Page Down> and <Page Up>.
<(Click next to continue)>]],
			intro5 =  [[If you have not done so already, select your Commander. Then <left click> on the <outpost> icon at the bottom build bar and click nearby your commander to build it.]],
			...
			selectBotLab = [[Select your <Outpost> by <left clicking> on it (look for the flashing blue circles).]],
			selectCon = [[Select your builder by <left clicking> on it (look for the flashing blue circles).]],
			guardFac = [[After you build a <bot plant> with your outpost, have the <outpost> guard the factory by first selecting it, then right clicking on the factory. The outpost will assist it until you give it a different order.]],
			tutorialEnd = [[This is the end of the tutorial. Check out the <campaign> for a more thorough introduction to TAP. I can be re-enabled in the <Menu (F10)> under <Help>. Goodbye!
...
<(Click next to continue)>]],
	...
	fr = {
		...
		steps = {
			intro = [[Bonjour! Je suis Nubtron, votre amical compagnon robot multilingue! Je vais vous apprendre à jouer a TAP. <(Cliquez ici pour continuer)>]],
		...
		steps = {
			intro = [[Terve! Minä olen Nubtron, ystävällinen robottiapurisi. Minä opetan sinua pelaamaan TAP:ia opastetun harjoittelun avulla. <(Paina tästä jatkaaksesi)>]],
	...
	my = {
		...
		steps = {
			intro = [[Selamat datang! Saya ialah Nubtron, sahabat robot anda yang berbilang bahasa. Saya akan mengajar anda cara bermain TAP. <(Klik Next untuk teruskan)>]],
	...
	pl = {
		...
		steps = {
			intro = [[Dzien dobry! Jestem Nubtron, wielojezyczny, przyjacielski robot. Naucze cie jak grac w TAP. <(Nacisnij tutaj aby kontynuowac)>]],

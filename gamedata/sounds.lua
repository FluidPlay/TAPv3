...
local files = VFS.DirList("sounds/reply/")
for i=1,#files do
	local fileName = files[i]
	-- Let's skip the folder (no .wav extension)
	local idx = string.find(fileName, ".wav")
	if type(idx) == "number" then
		fileNames = string.sub(fileName, 8, idx -1)
		t[fileNames] = {
			file     = fileName;
			gain = 1.2*0.3,
			pitchmod = 0.01,
...
--			Spring.Echo("Adding sound: "..pathPart)
